      ...
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div class="min-w-0">
          ...
          <p class="mt-1 text-sm text-slate-400">
            Polling every 5s ·
            <span class="text-emerald-400/80">{{ healthyLiveCount }} live</span>
          </p>
        </div>
        <button
          type="button"
...
/** Strict Healthy count — Offline / Degraded apps do not count as live */
const healthyLiveCount = computed(() =>
  pulses.value.filter(p => p.status === 'Healthy').length,
)
